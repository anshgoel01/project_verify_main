...
async function verifySubmission(supabase: any, submission: any, userName: string): Promise<{
  ...
  // Scrape certificate link for name/course, and project link for level
  const { name: courseraName, course: courseraCourse } = await scrapeCoursera(submission.coursera_link);
  
  // Use project_link as source of truth for level detection
  let detectedLevel = "Beginner";
  if (submission.project_link) {
    const { level: projectLevel } = await scrapeCoursera(submission.project_link);
    detectedLevel = projectLevel;
    console.log("Level detected from project link:", detectedLevel);
  } else {
    // Fallback to certificate link level
    const { level: certLevel } = await scrapeCoursera(submission.coursera_link);
    detectedLevel = certLevel;
    console.log("Level detected from certificate link (fallback):", detectedLevel);
  }
